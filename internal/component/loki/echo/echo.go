...
func (c *Component) ConsumeEntry(ctx context.Context, entry loki.Entry) error {
	...
	if err != nil {
		c.opts.SLogger.Error("failed to marshal structured metadata", "error", err)
		structured_metadata = []byte("{}")
	}

	c.opts.SLogger.Info("received log entry", "entry", entry.Line, "entry_timestamp", entry.Timestamp, "labels", entry.Labels.String(), "structured_metadata", string(structured_metadata))
	return nil
}
